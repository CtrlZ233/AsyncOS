...
pub fn translated_context(token: usize, context_va:usize ) -> usize {
    let page_table = PageTable::from_token(token);
    let mut va = context_va as usize;

    let cx: usize = *(page_table.translate_va(VirtAddr::from(va)).unwrap().get_mut());
