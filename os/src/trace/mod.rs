use crate::config::{MEMORY_END, TRACE_START};
// S trap
...
pub fn init() {
    unsafe {
        (TRACE_START as *mut usize).write_volatile(TRACE_START + 16);
        ((TRACE_START + 8) as *mut u32).write_volatile(0xbaad_f00d);
        ((TRACE_START + 12) as *mut u32).write_volatile(0xdead_beef);
    }
}
...
pub fn trace_test() {
    ...
    let trace_end = unsafe { (TRACE_START as *mut usize).read_volatile() };
    let trace_magic = unsafe { ((TRACE_START + 8) as *mut usize).read_volatile() };
